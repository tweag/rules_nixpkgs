...
def stardoc(
        ...
        out,
        input,
        aspect_template = "@rules_nixpkgs_docs//:templates/aspect.vm",
        func_template = "@rules_nixpkgs_docs//:templates/func.vm",
        header_template = "@rules_nixpkgs_docs//:templates/header.vm",
        provider_template = "@rules_nixpkgs_docs//:templates/provider.vm",
        rule_template = "@rules_nixpkgs_docs//:templates/rule.vm",
        **kwargs):
    _stardoc(
        name = name,
        out = out,
...
def copy_files(name, data):
    ...
    native.sh_binary(
        name = name,
        srcs = ["@rules_nixpkgs_docs//:copy-files.sh"],
...
def compare_files(name, data, error_message = ""):
    ...
    native.sh_test(
        name = name,
        srcs = ["@rules_nixpkgs_docs//:compare-files.sh"],
